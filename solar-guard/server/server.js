...
// อ่านพลาดติดกันกี่รอบถึงจะถือว่า "ขาดจริง" แล้วต้องบอกคนตอนกลับมา
// 24 รอบ x 5 วินาที = 2 นาที — สั้นกว่านี้จะเตือนบ่อยเกินไปเวลาเน็ตสะดุดแวบเดียว
const FAIL_ALERT_TICKS = 24;
...
async function tick() {
  ...
  try {
    ...
    /**
     * เพิ่งกลับมาจากช่วงที่อ่านไม่ได้ — ต้องบอกคน
     *
     * ตอนขาดจะส่งข้อความไม่ได้อยู่แล้ว (ถ้าเน็ตหลุดก็หลุดทั้งขาเข้าขาออก)
     * เลยต้องรอจนกลับมาแล้วค่อยบอกย้อนหลังว่าตาบอดไปกี่นาที
     *
     * ที่ต้องบอกเพราะช่วงที่ขาด ระบบเฝ้าเพดานให้ไม่ได้เลย และค่าไฟกับพีคของ
     * ช่วงนั้นจะหายไปจากการคำนวณ ทำให้ตัวเลขทั้งเดือนต่ำกว่าความจริง
     * — 4 ส.ค. 69 ปลั๊กเราเตอร์หลุด ระบบตาบอด 5 นาทีโดยไม่มีใครรู้
     */
    if (consecutiveFails >= FAIL_ALERT_TICKS) {
      const downMin = Math.round((consecutiveFails * L.readEverySec) / 60);
      log(`อ่านอินเวอร์เตอร์ได้อีกครั้ง หลังขาดไป ${downMin} นาที`);
      if (!NO_ALERTS) {
        await relay(
          `📡 <b>ระบบกลับมาอ่านค่าได้แล้ว</b>\n${cfg.siteName} • ${hhmm()} น.\n`
          + `เมื่อกี้อ่านอินเวอร์เตอร์ไม่ได้นาน <b>${downMin} นาที</b>\n\n`
          + 'ช่วงที่ขาดไปนั้นระบบเฝ้าเพดานให้ไม่ได้ และค่าไฟช่วงนั้นจะไม่ถูกนับ\n'
          + '<i>สาเหตุที่พบบ่อย: เราเตอร์หรือสวิตช์ไม่มีไฟ / สาย LAN หลุด / อินเวอร์เตอร์รีสตาร์ท</i>',
          { toBoss: true, silent: true },
        );
      }
    }

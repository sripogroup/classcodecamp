...
3. **เปิด Modbus TCP ที่ SmartDongle** ⚠️ **ตั้งจากโรงงานมาเป็นปิดไว้ ต้องเปิดเองเสมอ**

   > **ตั้งที่ SmartDongle ไม่ใช่ที่ตัวอินเวอร์เตอร์** — นี่คือจุดที่คนพลาดกันบ่อยที่สุด

   **ทางที่ 1 — หน้าเว็บ FusionSolar**
   Device Management → **ติ๊กช่องหน้า SmartDongle** → **Set Parameters**
   → แท็บ **Modbus-TCP** → เลือก **Enable (unrestricted)** → กด **Set**

   **ทางที่ 2 — แอพมือถือ** (ถ้าหน้าเว็บไม่มีให้ตั้ง)
   เอกสารของ Huawei บางฉบับระบุว่าต้องตั้งจากในเครื่องเท่านั้น
   ให้ใช้แอพ FusionSolar หรือแอพ SUN2000 ต่อกับ WLAN ของอินเวอร์เตอร์เอง

   เงื่อนไข: เฟิร์มแวร์ SmartDongle ต้องเป็น V100R001C00SPC127 ขึ้นไป

   > บางประเทศถือว่าพอร์ต 502 เป็นพอร์ตเสี่ยง Huawei เลยแนะนำให้ย้ายไป **1502**
   > ถ้าตั้งเป็น 1502 ตอนรันสคริปต์ให้เติม `-Port 1502` ด้วย

### ผลตรวจจริงของโรงงานนี้ (2 ส.ค. 2569)

| สิ่งที่ตรวจ | ผล |
|---|---|
| หาอุปกรณ์ในวง LAN | เจอ `192.168.1.26` เปิดพอร์ต 502 |
| MAC address | `1C:43:63:59:8D:A3` → **Huawei Technologies** (ยืนยันจากฐานข้อมูล OUI ของ nmap) |
| อุปกรณ์ตัดสายทิ้งหลังไม่ส่งอะไร | **236 ms** — สั้นผิดปกติ Modbus server จริงไม่ทำแบบนี้ |
| ส่งคำขอ Modbus (ลอง 5 จังหวะ × 2 unit id) | ส่งออกไปได้ทุกครั้ง แต่ **อุปกรณ์ปิดสายโดยไม่ตอบ** ทุกครั้ง |

**สรุป:** ไม่ใช่ปัญหาจังหวะเวลา และไม่ใช่ unit id ผิด — ถ้า Modbus ทำงานอยู่
มันต้องตอบกลับ หรืออย่างน้อยตอบเป็น exception แต่นี่คือรับข้อมูลแล้วปิดสายเงียบ ๆ
ซึ่งเป็นอาการของ **บริการ Modbus ยังไม่ได้เปิด** พอร์ต 502 ที่เห็นเป็นของบริการ
SDongle ที่คุยกับคลาวด์ Huawei ไม่ใช่ Modbus สำหรับให้เราอ่าน

### 🛑 สาเหตุที่แท้จริง: เฟิร์มแวร์ SDongle เก่าเกินไป

อุปกรณ์จริงของโรงงานนี้ (อ่านจากหน้า Device Management):

| อุปกรณ์ | รุ่น | เฟิร์มแวร์ | SN |
|---|---|---|---|
| Dongle | **SDongleA-05** | **V100R001C00SPC116** | HV2070010866 |
| Inverter | **SUN2000-36KTL-M3** | V100R001C20SPC122 | 6T2139026297 |

**Modbus TCP ต้องใช้ SDongle เฟิร์มแวร์ V100R001C00SPC127 ขึ้นไป**

ของที่นี่คือ **SPC116** ซึ่งต่ำกว่า **SPC127** → **เฟิร์มแวร์รุ่นนี้ไม่มีฟีเจอร์ Modbus TCP เลย**

อธิบายทุกอย่างที่เจอมาได้ครบ:
- พอร์ต 502 เปิดอยู่ (เป็นบริการที่ SDongle ใช้คุยกับคลาวด์) แต่ไม่ตอบ Modbus
- ในหน้าเว็บไม่มีแท็บ Modbus-TCP ให้ตั้ง เพราะฟีเจอร์ไม่มีอยู่ในเฟิร์มแวร์นี้

**ทางแก้ (เลือกอย่างใดอย่างหนึ่ง):**
1. ให้ดีลเลอร์อัปเกรดเฟิร์มแวร์ SDongle เป็น SPC127 ขึ้นไป แล้วเปิด Modbus TCP
2. ขอบัญชี Northbound API จากดีลเลอร์แทน (ไม่ต้องแตะเฟิร์มแวร์)
3. ติดมิเตอร์แยกของตัวเอง ไม่พึ่ง Huawei เลย (ดู `../docs/AUTOSHED.md` หัวข้อ 3)
